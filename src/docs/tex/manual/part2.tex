...
AMGx does not scale well with MPI processes because, for one, it does not seem to have a coarse grid solver.
The PETSc parameter interface to AMGx is fragile and not flexible, one must specify an AMGx input file with \trl{-c path-to-intput-file}.
The classical AMG method is recomended (eg, -c share/petsc/amgx/AMG_CLASSICAL_AGGRESSIVE_L1_RT6.json).
